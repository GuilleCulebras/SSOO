...
copydir(char *dOrigen, char *dDest, mode_t perD, mode_t perF){
	...
	for(;;){
		...
		sprintf(pathOrigen,"%s/%s",dOrigen,datos_dir->d_name); //path completo de origen
		sprintf(pathDest,"%s/%s",dDest,datos_dir->d_name); //path completo destino
		...
		if (datos_dir->d_name[0] != '.'){
        	...
        	if ((st.st_mode & S_IFMT)==S_IFDIR){ //si es directorio
				if(mkdir(pathDest, 0777) < 0) //creamos directorio con el path completo destino
					err(1,"error: mkdir %s",pathDest);

				copydir(pathOrigen,pathDest,perD,perF);	//repetimos el procedimiento    	
...
main(int argc, char *argv[]){	
	...
	perDir = strtol(argv[1],0,8); // guardamos los permisos del directorio en base octal
	perFile = strtol(argv[2],0,8); //guardamos los permisos del fichero en base octal

	if (stat(argv[3], &st)<0)
		err(1,"error: stat");

	if ((st.st_mode & S_IFMT)==S_IFREG) //Si es fichero
